  ...
  if [ ! -d $f ]; then
    echo "Processing $f file..."
      curl -X PUT -T $f -u ${BINTRAY_USER}:${BINTRAY_API_KEY} \
	      https://api.bintray.com/content/${BINTRAY_OWNER}/${BINTRAY_REPO}/${PCK_NAME}/${PCK_VERSION}/$f;publish=0;override=1
    echo ""
  fi
done
...
curl -X POST -u ${BINTRAY_USER}:${BINTRAY_API_KEY} \
    https://api.bintray.com/content/${BINTRAY_OWNER}/${BINTRAY_REPO}/${PCK_NAME}/${PCK_VERSION}/publish -d "{ \"discard\": \"false\" }"
