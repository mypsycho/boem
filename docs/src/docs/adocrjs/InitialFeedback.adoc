...
EmfContribution provides a factory to initialize 
extensions of EPackage into the singleton.

Major functions (edit, validated, etc) supported by
1 engine based on EmfContribution keys and running with an EmfStrecher.
...
=== Reverse engineer of existing models

Applicable to genmodel, sirius, test, ...

EReversIt can generate Xtend classes matching any 
EMF model.
